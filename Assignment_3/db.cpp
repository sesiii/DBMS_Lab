...
void executeSelectQuery(pqxx::connection &conn, const string &query, int queryNum)
{
    try
    {
        pqxx::work txn(conn);
        ...
        // cout << "\n--- Executing SELECT Query " << queryNum << " ---\n";
        if (res.empty())
        {
            cout << "Query " << queryNum << ": No results found.\n";
            txn.commit();
            return;
        }

        // Determine column widths.
        vector<int> columnWidths(res.columns(), 0);
        for (size_t i = 0; i < res.columns(); ++i)
        {
            columnWidths[i] = string(res.column_name(i)).length();
        }
        for (const auto &row : res)
        {
            for (size_t i = 0; i < row.size(); ++i)
            {
                string value = row[i].is_null() ? "NULL" : row[i].c_str();
                columnWidths[i] = max(columnWidths[i], static_cast<int>(value.length()));
            }
        }
        ...
        for (size_t i = 0; i < res.columns(); ++i)
        {
            cout << " " << setw(columnWidths[i]) << left << res.column_name(i) << " |";
        }
        cout << "\n";
        printTableLine(columnWidths);
        for (const auto &row : res)
        {
            ...
            {
                string value = row[i].is_null() ? "NULL" : row[i].c_str();
                cout << " " << setw(columnWidths[i]) << left << value << " |";
            }
            cout << "\n";
        }
        printTableLine(columnWidths);
    ...
    {
        cerr << "Query " << queryNum << " failed: " << e.what() << "\n";
    }
}
...
void executeNonSelectQuery(pqxx::connection &conn, const string &query, int queryNum, const string &successMessage)
{
    try
    {
        ...
        txn.exec(query);
        txn.commit();
        // cout << "Query " << queryNum << " executed successfully: " << successMessage << "\n";
    }
    catch (const exception &e)
    {
        cerr << "Query " << queryNum << " failed: " << e.what() << "\n";
...
int main()
{
    try
    {
        ...
        cout << "Dropping existing tables (if any)...\n";
        for (const auto &q : dropQueries)
        {
            ++queryNum;
        ...
        vector<pair<string, string>> createQueries = {
        ...
        for (const auto &p : createQueries)
        {
            ++queryNum;
        ...
        vector<pair<string, string>> insertQueries = {
            ...
            {"INSERT INTO land_records VALUES "
             ...
             "(5, 11, 2.0, 'Wheat'), "
             "(6, 12, 1.2, 'Rice');",
             "Data inserted into Land Records."},
            ...
            {"INSERT INTO panchayat_employees VALUES "
             ...
             "(4, 14, 'Clerk');",
             "Data inserted into Panchayat Employees."},
            ...
            {"INSERT INTO welfare_schemes VALUES "
             ...
             "Data inserted into Welfare Schemes."},
            ...
            {"INSERT INTO scheme_enrollments VALUES "
             ...
             "(4, 2, 4, '2022-09-05'), "
             "(5, 5, 5, '2022-10-12');",
             "Data inserted into Scheme Enrollments."},
            ...
            {"INSERT INTO census_data VALUES "
             ...
             "Data inserted into Census Data."},
            ...
            {"INSERT INTO expenditure VALUES "
             ...
             "Data inserted into Expenditure."},
            ...
            {"INSERT INTO service_requests VALUES "
             ...
             "Data inserted into Service Requests."}};

        cout << "Inserting data into tables...\n";
        for (const auto &p : insertQueries)
        {
            ++queryNum;
            executeNonSelectQuery(conn, p.first, queryNum, p.second);
        }
        ...
        cout << "Executing SELECT queries...\n";
        for (const auto &p : selectQueries)
        {
            ++queryNum;
            // p.second contains the query label, e.g., "Query 1", "Query 2", etc.
            cout << "\n=== " << p.second << " ===\n";
            executeSelectQuery(conn, p.first, queryNum);
        }
    }
    catch (const exception &e)
    {
