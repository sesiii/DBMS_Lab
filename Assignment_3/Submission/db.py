...
def print_table(headers, rows):
    """Prints table results using tabulate."""
    print(tabulate(rows, headers=headers, tablefmt="grid"))

def execute_non_select_query(cursor, query, query_num, success_message):
    """Executes a non-SELECT SQL command (DROP, CREATE, INSERT)."""
    try:
        cursor.execute(query)
        # print(f"Query {query_num} executed successfully: {success_message}")
    except Exception as e:
        print(f"Query {query_num} failed: {e}")

def execute_select_query(cursor, query, query_num):
    """Executes a SELECT query and prints its results in a formatted table."""
    try:
        cursor.execute(query)
        rows = cursor.fetchall()
        if not rows:
            print(f"Query {query_num}: No results found.")
        else:
...
def main():
    try:
        ...
        # Use autocommit mode so that each query is immediately committed.
    ...
    for q in drop_queries:
        query_num += 1
        execute_non_select_query(cursor, q, query_num, "Table dropped (if existed).")
    ...
    create_queries = [
        ...
        ("""CREATE TABLE scheme_enrollments(
            ...
            );""", "Scheme Enrollments table created."),
        ...
        ("""CREATE TABLE service_requests(
              ...
              status VARCHAR(200)
            );""", "Service Requests table created.")
    ]

    print("Creating tables...")
    for query, msg in create_queries:
        query_num += 1
        execute_non_select_query(cursor, query, query_num, msg)
    ...
    insert_queries = [
        ("""INSERT INTO households VALUES 
             ...
             (10, 'Green Avenue, Ward 1', 6, 220000);""", "Data inserted into Households."),
        ("""INSERT INTO citizens VALUES 
             ...
             (14, '147258369014', 'Raj', 'Patel', '2020-05-05', '8899001122', 3, 'General', 'Male', 2, NULL, 'Child', 'Single');""", "Data inserted into Citizens."),
        ("""INSERT INTO land_records VALUES 
             ...
             (4, 9, 1.5, 'Rice'), 
             (5, 11, 2.0, 'Wheat'), 
             (6, 12, 1.2, 'Rice');""", "Data inserted into Land Records."),
        ("""INSERT INTO panchayat_employees VALUES 
             (1, 11, 'Sarpanch'), 
             (2, 5, 'Secretary'), 
             (3, 3, 'Revenue Collector'), 
             (4, 14, 'Clerk');""", "Data inserted into Panchayat Employees."),
        ...
        ("""INSERT INTO welfare_schemes VALUES 
             ...
             (5, 'Pradhan Mantri Matru Vandana Yojana', 'Maternity Benefit Programme', 'Pregnant and Lactating Women', 'Cash Incentive ₹5000', '2017-09-01', NULL);""", "Data inserted into Welfare Schemes."),
        ("""INSERT INTO scheme_enrollments VALUES 
             (1, 1, 1, '2022-06-15'), 
             (2, 3, 2, '2022-07-20'), 
             (3, 4, 3, '2022-08-10'), 
             (4, 2, 4, '2022-09-05'), 
             (5, 5, 5, '2022-10-12');""", "Data inserted into Scheme Enrollments."),
        ("""INSERT INTO census_data VALUES 
             ...
             (5, 5, 6, 2023, 3, 'Young Professional Family', 'Household Census Data', 'Population Survey', '2023-03-19');""", "Data inserted into Census Data."),
        ("""INSERT INTO tax VALUES 
             ...
             (5, 6000.00, '2023-07-25', 'Pending', 'Cheque', '2023-2024', 5);""", "Data inserted into Tax."),
        ("""INSERT INTO income VALUES 
             (1, 50000.00, '2024-04-15', 'Government Subsidy', '2023-2024'), 
             (2, 75000.00, '2024-05-10', 'Agricultural Sale', '2023-2024'), 
             (3, 60000.00, '2024-06-20', 'Rent', '2023-2024');""", "Data inserted into Income."),
        ("""INSERT INTO expenditure VALUES 
             ...
             (3, 25000.00, '2024-06-10', 'Household Repairs', 'Maintenance', 'Online', '2023-2024');""", "Data inserted into Expenditure."),
        ("""INSERT INTO service VALUES 
             (1, 'Internet', 'Active', '2024-03-10'), 
             (2, 'Electricity', 'Inactive', '2024-02-05'), 
             (3, 'Water Supply', 'Active', '2024-01-20');""", "Data inserted into Service."),
        ("""INSERT INTO service_requests VALUES 
             (1, 1, 1, '2024-03-15', 'Completed'), 
             (2, 2, 2, '2024-02-10', 'Pending'), 
             (3, 3, 3, '2024-01-25', 'Completed');""", "Data inserted into Service Requests.")
    ]

    print("\nInserting data into tables...")
    for q, msg in insert_queries:
        query_num += 1
        execute_non_select_query(cursor, q, query_num, msg)
    ...
    select_queries = [
        ...
        ("""SELECT COUNT(*) AS total_street_lights 
             ...
             AND EXTRACT(YEAR FROM installation_date) = 2024;""", "Query 7"),
        ("""SELECT COUNT(*) AS total_vaccinations 
             ...
             AND parent.educational_qualification = '10th Class';""", "Query 8"),
        ("""SELECT COUNT(*) AS total_boy_births 
             FROM citizens 
             WHERE gender = 'Male' 
             AND EXTRACT(YEAR FROM date_of_birth) = 2024;""", "Query 9"),
    ...
    print("Executing SELECT queries...")
    for q, label in select_queries:
        query_num += 1
        print(f"\n=== {label} ===")
        execute_select_query(cursor, q, query_num)
